<template>
  <div class="home-container">
    ...
    <div class="home-wrapper">
      ...
      <terminalIntercom v-else-if="currentTab == 3" ref="intercom"></terminalIntercom>
      <policeFingerprint v-else-if="currentTab == 4" ref="policeFingerprint"></policeFingerprint>
      <terminalFace v-else-if="currentTab == 5" ref="face"></terminalFace>
      <terminalCheck v-else-if="currentTab == 6" ref="check"></terminalCheck>
      ...
      <terminalDiagnosis v-else-if="currentTab == 9" ref="diagnosis"></terminalDiagnosis>
      <terminalShopping v-else-if="currentTab == 10" ref="shopping"></terminalShopping>
    ...
    <div class="neil-modal-container">
      ...
      <neil-modal :show="showManagerCall">
        <div class="common-modal-container">
          ...
          <div class="common-modal-button">
            <div class="btn-cancel" @click="closeModal('ManagerCall')">取消</div>
            <div class="btn-confirm" @touchstart.stop="handleManagerCall">确定</div>
          </div>
        </div>
      </neil-modal>
      <!-- 监仓视频通话弹框 -->
      <neil-modal :show="showTerminalCall">
        <div class="common-modal-container">
          ...
          <div class="common-modal-button">
            <div class="btn-cancel" @click="closeModal('TerminalCall')">取消</div>
            <div class="btn-confirm" @touchstart.stop="handleTerminalCall">确定</div>
          </div>
        </div>
      </neil-modal>
      <!-- 视频通话连接 -->
      ...
      <!-- 登录弹框 -->
      <recognition-dialogs ref="recognitionDialogs" useFor="login" :isShow="showRecognitionDialogs" :role="loginRole" @fingerRecognitionSuccess="fingerRecognitionSuccess" @faceRecognitionSuccess="faceRecognitionSuccess" @close="handleRecognitionDialogsClose"></recognition-dialogs>
      <!-- 同步民警指纹弹框 -->
      <neil-modal :show="showPoliceFinger">
        <div class="common-modal-container">
          ...
          <div class="common-modal-button">
            ...
            <div class="btn-confirm" @touchstart.stop="policeFingerSync">确定</div>
          </div>
        </div>
      </neil-modal>
      <!-- 设备重启弹框 -->
...
export default {
  ...
  onLoad () {
    ...
    if (isNullStr(uni.getStorageSync("baseUrl"))) {
      ...
      this.terminalIP = base.getIpAddress().ip;
      // 同步配置信息
      this.configInfoSync();
      // #ifdef APP-PLUS
      const _this = this;
      ...
      plus.globalEvent.addEventListener("onCallStateChanged", (e) => {
        ...
        if (e.state === 1) {
          ...
          if (sip.answer(e.address) === 0) {
            ...
            _this.sendWebsocket(`{maindevno:"${controlCode}",devno:"${managerCode}",type:"100",msg:"20"}`);
        ...
        if (e.state === 6) {
          ...
          _this.sendWebsocket(`{maindevno:"${managerCode}",devno:"${terminalCode}",type:"100",msg:"23"}`);
          _this.handleManagerIntercom(_this);
        }
        // 主机声音逻辑，接通后
        if (e.state === 7) {
          ...
          if (!_this.micState) {
            _this.micState = true;
            getApp().globalData.harUtils.setAdcBoost(uni.getStorageSync("audioMICValue"));
          }
        }
        // 对讲关闭或出错
  ...
  methods: {
    ...
    handleManagerIntercom (_this) {
      ...
      getApp().globalData.harUtils.setVolume(0, uni.getStorageSync("mediaDefaultVolume"));
    ...
    showStatusBar () {
      getApp().globalData.harUtils.showStatusBar();
      Log.writeLog("【手动打开状态栏，showStatusBar()】", false);
    },
    //隐藏状态栏
    hideStatusBar () {
      getApp().globalData.harUtils.hideStatusBar();
      Log.writeLog("【手动关闭状态栏，hideStatusBar()】", false);
    },
    // 同步配置信息
    configInfoSync () {
      this.sysCacheInfo = '';
    ...
    echoCalibrate () {
      ...
      Log.writeLog("【回声校准成功，echoCalibrate()】", false);
    },
    // 开启守护
    openCrashHandle () {
    ...
    async setAllBindInfo () {
      ...
      let res = await Api.apiCall("get", Api.index.getTerminalInfo, params, true);
      if (res.state.code == "200") {
    ...
    setManagerStorage (data) {
      let managerInfo = {
        ...
        TerSipAccount: data.TerSipAccount,
        embeddedIp: data.embeddedIp,
    ...
    connectWebSocketInit (code) {
      ...
      this.socketTask.onError((err) => {
        console.log("连接失败，可能是websocket服务不可用，正在发起重连", JSON.stringify(err));
        this.webSocketReConnct();
      });
      // 关闭WebSocket
      ...
      this.socketTask.onMessage((res) => {
        ...
        if (info.type == this.$config.controlType.HEARTBEAT) {
          let code = uni.getStorageSync("managerInfo").code;
          let ip = uni.getStorageSync("managerInfo").ip;
          this.sendWebsocket(
          ...
          if (info.msg == "1") {
            ...
            // 开启监视监听
          } else if (info.msg == "6" || info.msg == "10" || info.msg == "13" || info.msg == "21") {
            // 视频通话无应答
            this.intercomState = false;
            this.showVideoConnect = false;
          ...
          if (info.msg == "7") {
            console.log(getApp().globalData.harUtils.switchCtrl(0, 0));
            getApp().globalData.harUtils.switchCtrl(0, 0);
          } else if (info.msg == "8") {
            console.log(getApp().globalData.harUtils.switchCtrl(0, 1));
          ...
          Log.writeLog("this.$config.controlType.DEVICE,打开openAudioIO", false);
          if (info.msg == "0") {
            console.log("开始语音播报");
            base.speech(content);
    ...
    webSocketReConnct () {
      ...
      this.socketTimer = setTimeout(() => {
        console.log('create，清除this.socketTimer定时器，触发重连机制', uni.getStorageSync("managerInfo").code);
        this.connectWebSocketInit(uni.getStorageSync("managerInfo").code);
      }, this.websocketTime);
    },
    webSocketOff () {
      ...
      setTimeout(() => {
        if (this.isOpen) {
          ...
          for (let i = 0; i < ids.length; i++) {
            ...
            Log.writeLog("this.$config.controlType.DEVICE,删除在押人员指纹ID:" + ids[i], false);
            res++;
          }
          if (res > 0) {
    ...
    async getPoliceFingerInfo () {
      ...
      if (res.state.code == "200") {
        ...
        res.data.map((item) => {
          if (!!item.mKey) {
            let result1 = getApp().globalData.fingerprint.setTemplate(item.mKey, str2Array(item.pHint));
            Log.writeLog("同步民警指纹信息setTemplate()key:" + item.mKey, false);
            if (!result1) {
              successNum++;
          ...
          if (!!item.mKey2) {
            let result2 = getApp().globalData.fingerprint.setTemplate(item.mKey2, str2Array(item.pHint2));
            Log.writeLog("同步民警指纹信息setTemplate()key:" + item.mKey2, false);
            if (!result2) {
              successNum++;
        ...
        Log.writeLog(`同步民警指纹成功${successNum}个，失败${failNum}个`, false);
        this.handleShowToast(`同步民警指纹成功${successNum}个，失败${failNum}个`);
        getApp().globalData.fingerprint.close();
    ...
    async saveFingerStateInfo (content, type) {
      let res = await Api.apiCall("post", Api.index.saveFingerState + `?content=${content}&logType=${type}`, null);
      if (res.state.code == "200") {
    ...
    handleManagerCall () {
      if (!this.isRepeatState) {
        ...
        this.sendWebsocket(`{maindevno:'${controlCode}', devno:'${managerCode}', type:'100', msg:'19', extend:{'roomName':'${manageInfo.roomName}仓外屏','sipAccount':'${manageInfo.sipAccount}'}}`);
        this.voiceBroadcast("正在发起视频通话");
      }
    },
    // 监仓视频通话
    ...
    async terminalDeviceStatus (code, controlCode) {
      let res = await Api.apiCall(
        "get",
        Api.index.terminalDeviceStatus + `?code=${code}&controlCode=${controlCode}`,
        null
      );
      if (res.state.code == "200") {
    ...
    closeModal (type) {
      ...
      if (type == "VideoConnect") {
        ...
        let managerCode = uni.getStorageSync("managerInfo").code;
        let controlCode = uni.getStorageSync("managerInfo").controlCode;
        this.sendWebsocket(`{maindevno:"${controlCode}",devno:"${managerCode}",type:"100",msg:"25"}`);
      }
    },
    // 切换登录模式回调
